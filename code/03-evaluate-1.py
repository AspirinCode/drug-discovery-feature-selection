...
Result:
SVM Accuracy: 0.9910
SVM + WM Accuracy: 0.9900
SVM + RFE Accuracy: 0.9909
...
result_file = '03-evaluate-1_result.csv'
methods = [
    {'name': 'SVM + RFE', 'filename': 'SVM_RFE_features_mask.json'},
    {'name': 'SVM + WM', 'filename': 'WM_GA_SVM_features_mask.json'},
    {'name': 'SVM', 'filename': None},
]
roc_chart_file = '03-evaluate-1_roc_chart.png'
scores_chart_file = '03-evaluate-1_scores_chart.png'
verbosity = 0
...
if len(results) > 0:
    ...
    for i, row in enumerate(results):
        print('{} Accuracy: {:.4f}'.format(row['name'], row['mean_acc']))
    ...
    plt.title('ROC on PubChem Dataset')
    ...
    plt.ylim([0.950, 1.000])
    plt.ylabel('Scores')
    plt.title('Classification Metrics Comparison on PubChem Dataset')
